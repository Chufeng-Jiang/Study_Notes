...
void *threadpool_thread(void *threadpool)
{
    ...
    while (true) 
	{
		...
		{  
            ...
            if (pool->wait_exit_thr_num > 0) // 有需要让子线程退出的
			{
...
int threadpool_destroy(threadpool_t *pool)
{
    ...
    pool->shutdown = true; //设置成true所有子线程都退出
...
int is_thread_alive(pthread_t tid) //异常死掉的情况，线程死了，但是值还没改为0
{
...
int main(void)
{
    ...
    for (i = 0; i < 20; i++) 
	{
		...
		//如果不用sleep，还可以设置分离属性，然后调用thread_join
    sleep(10);                                          /* 等子线程完成任务 */
